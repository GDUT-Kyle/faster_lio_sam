...
struct KD_TREE_NODE
{
    ...
    // 左右子节点
    KD_TREE_NODE<PointType> *left_son_ptr = nullptr;
    KD_TREE_NODE<PointType> *right_son_ptr = nullptr;
    // 父节点
    KD_TREE_NODE<PointType> *father_ptr = nullptr;
...
struct PointType_CMP{
    PointType point;
    float dist = 0.0;
    PointType_CMP<PointType> (PointType p = PointType(), float d = INFINITY){
        this->point = p;
        this->dist = d;
    };
    bool operator < (const PointType_CMP<PointType> &a)const{
...
class MANUAL_Q{
    private:
        ...
        vector<Operation_Logger_Type<PointType>> q;
        bool is_empty;
    public:
        MANUAL_Q(){
            q.resize(Q_LEN);
        ...
        void pop();
        Operation_Logger_Type<PointType> front();
        Operation_Logger_Type<PointType> back();
        void clear();
        void push(Operation_Logger_Type<PointType> op);
        bool empty();
        int size();
};
...
class MANUAL_HEAP
{
    public:
        ...
        ~MANUAL_HEAP();
        void pop();
        PointType_CMP<PointType> top();
        void push(PointType_CMP<PointType> point);
        int size();
        void clear();
    private:
        PointType_CMP<PointType> * heap; // 存储堆元素的数组
...
private:
    ...
    MANUAL_Q<PointType> Rebuild_Logger;    
    PointVector Rebuild_PCL_Storage;
    KD_TREE_NODE<PointType> ** Rebuild_Ptr = nullptr;
    int search_mutex_counter = 0;
    static void * multi_thread_ptr(void *arg);
    void multi_thread_rebuild();
    void start_thread();
    void stop_thread();
    void run_operation(KD_TREE_NODE<PointType> ** root, Operation_Logger_Type<PointType> operation);
    ...
    KD_TREE_NODE<PointType> * STATIC_ROOT_NODE = nullptr;
    PointVector Points_deleted;
    PointVector Downsample_Storage;
    PointVector Multithread_Points_deleted;
    void InitTreeNode(KD_TREE_NODE<PointType> * root);
    void Test_Lock_States(KD_TREE_NODE<PointType> *root);
    void BuildTree(KD_TREE_NODE<PointType> ** root, int l, int r, PointVector & Storage);
    void Rebuild(KD_TREE_NODE<PointType> ** root);
    int Delete_by_range(KD_TREE_NODE<PointType> ** root, BoxPointType boxpoint, bool allow_rebuild, bool is_downsample);
    void Delete_by_point(KD_TREE_NODE<PointType> ** root, PointType point, bool allow_rebuild);
    void Add_by_point(KD_TREE_NODE<PointType> ** root, PointType point, bool allow_rebuild, int father_axis);
    void Add_by_range(KD_TREE_NODE<PointType> ** root, BoxPointType boxpoint, bool allow_rebuild);
    void Search(KD_TREE_NODE<PointType> * root, int k_nearest, PointType point, MANUAL_HEAP<PointType> &q, double max_dist);//priority_queue<PointType_CMP>
    void Search_by_range(KD_TREE_NODE<PointType> *root, BoxPointType boxpoint, PointVector &Storage);
    bool Criterion_Check(KD_TREE_NODE<PointType> * root);
    void Push_Down(KD_TREE_NODE<PointType> * root);
    void Update(KD_TREE_NODE<PointType> * root); 
    void delete_tree_nodes(KD_TREE_NODE<PointType> ** root);
    void downsample(KD_TREE_NODE<PointType> ** root);
    bool same_point(PointType a, PointType b);
    float calc_dist(PointType a, PointType b);
    float calc_box_dist(KD_TREE_NODE<PointType> * node, PointType point);    
...
public:
    ...
    void flatten(KD_TREE_NODE<PointType> * root, PointVector &Storage, delete_point_storage_set storage_type);
    void acquire_removed_points(PointVector & removed_points);
    BoxPointType tree_range();
    PointVector PCL_Storage;     
    KD_TREE_NODE<PointType> * Root_Node = nullptr;
...
template<typename PointType>
// 初始化根节点
void KD_TREE<PointType>::InitTreeNode(KD_TREE_NODE<PointType> * root){
    root->point.x = 0.0f;
    root->point.y = 0.0f;
    root->point.z = 0.0f;       
...
void KD_TREE<PointType>::multi_thread_rebuild(){
    ...
    // 新建虚拟头结点和重建的树的新节点
    KD_TREE_NODE<PointType> * father_ptr, ** new_node_ptr;
    pthread_mutex_lock(&termination_flag_mutex_lock);
    ...
    while (!terminated){
        ...
        if (Rebuild_Ptr != nullptr ){                    
            ...
            KD_TREE_NODE<PointType> * old_root_node = (*Rebuild_Ptr);                            
            ...
            Operation_Logger_Type<PointType> Operation;
            KD_TREE_NODE<PointType> * new_root_node = nullptr;  
            ...
            KD_TREE_NODE<PointType> * update_root = *Rebuild_Ptr;
            // 将子树属性上拉到树的根节点
...
template<typename PointType>
void KD_TREE<PointType>::run_operation(KD_TREE_NODE<PointType> ** root, Operation_Logger_Type<PointType> operation){
    switch (operation.op)
    {
    case ADD_POINT:      
...
void KD_TREE<PointType>::Build(PointVector point_cloud){
    ...
    STATIC_ROOT_NODE = new KD_TREE_NODE<PointType>;
    //初始化当前节点的属性
...
void KD_TREE<PointType>::Nearest_Search(PointType point, int k_nearest, PointVector& Nearest_Points, vector<float> & Point_Distance, double max_dist){   
    ...
    MANUAL_HEAP<PointType> q(2*k_nearest);
    q.clear();
    // 清空Point_Distance
    ...
    for (int i=0; i<PointToAdd.size();i++){
        ...
        if (downsample_switch){
            ...
            if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != Root_Node){  
                ...
                if (Downsample_Storage.size() > 1 || same_point(PointToAdd[i], downsample_result)){
                    Operation_Logger_Type<PointType>  operation_delete, operation;
                    // 记录待删除的点
...
template<typename PointType>
void KD_TREE<PointType>::BuildTree(KD_TREE_NODE<PointType> ** root, int l, int r, PointVector & Storage){
    if (l>r) return;
    //给**root分配内存
    *root = new KD_TREE_NODE<PointType>;
    // 初始化根节点
    InitTreeNode(*root);
    // 相当于mid=(1+r)/2
    int mid = (l+r)>>1;
    ...
    ******************************************************/
    ...
    KD_TREE_NODE<PointType> * left_son = nullptr, * right_son = nullptr;
...
// 重建树
template<typename PointType>
void KD_TREE<PointType>::Rebuild(KD_TREE_NODE<PointType> ** root){    
    KD_TREE_NODE<PointType> * father_ptr;
...
// box-wise delete
template<typename PointType>
int KD_TREE<PointType>::Delete_by_range(KD_TREE_NODE<PointType> ** root,  BoxPointType boxpoint, bool allow_rebuild, bool is_downsample){   
    ...
    Operation_Logger_Type<PointType> delete_box_log;
...
// 前序遍历，从树中删除指定点
template<typename PointType>
void KD_TREE<PointType>::Delete_by_point(KD_TREE_NODE<PointType> ** root, PointType point, bool allow_rebuild){   
    ...
    Operation_Logger_Type<PointType> delete_log;
    struct timespec Timeout;    
...
template<typename PointType>
void KD_TREE<PointType>::Add_by_range(KD_TREE_NODE<PointType> ** root, BoxPointType boxpoint, bool allow_rebuild){
    ...
    Operation_Logger_Type<PointType> add_box_log;
    struct timespec Timeout;    
...
template<typename PointType>
void KD_TREE<PointType>::Add_by_point(KD_TREE_NODE<PointType> ** root, PointType point, bool allow_rebuild, int father_axis){     
    // 如果当前节点为空的话，则新建节点
    if (*root == nullptr){
        // 新建节点并初始化
        *root = new KD_TREE_NODE<PointType>;  // 注意到root是指针的指针，所以不需要返回地址即可实现父节点指向当前新节点
        InitTreeNode(*root);
        // 配置节点的值
        (*root)->point = point;
        // 标记该节点的划分轴
    ...
    Operation_Logger_Type<PointType> add_log;
    struct timespec Timeout;    
    // 设置动作为ADD_POINT
...
// k近邻搜索，这是一个前序遍历的过程
template<typename PointType>
void KD_TREE<PointType>::Search(KD_TREE_NODE<PointType> * root, int k_nearest, PointType point, MANUAL_HEAP<PointType> &q, double max_dist){
    // 到达叶子节点则返回
    if (root == nullptr || root->tree_deleted) return;   
    ...
    if (!root->point_deleted){
        ...
        if (dist <= max_dist && (q.size() < k_nearest || dist < q.top().dist)){
            ...
            PointType_CMP<PointType> current_point{root->point, dist};     
            // 压入堆中               
...
template<typename PointType>
void KD_TREE<PointType>::Search_by_range(KD_TREE_NODE<PointType> *root, BoxPointType boxpoint, PointVector & Storage){
    if (root == nullptr) return;
...
template<typename PointType>
bool KD_TREE<PointType>::Criterion_Check(KD_TREE_NODE<PointType> * root){
    ...
    KD_TREE_NODE<PointType> * son_ptr = root->left_son_ptr;
...
template<typename PointType>
void KD_TREE<PointType>::Push_Down(KD_TREE_NODE<PointType> *root){
    if (root == nullptr) return;
    // 实例化一个操作记录器
    Operation_Logger_Type<PointType> operation;
    // 操作器设置为PUSH_DOWN状态
...
// 树上所有点坐标轴的范围，其中k为点维度。
template<typename PointType>
void KD_TREE<PointType>::Update(KD_TREE_NODE<PointType> * root){ // 该函数不需要递归，因为父节点的属性只与左右子节点相关
    // 该节点的左右子节点
    KD_TREE_NODE<PointType> * left_son_ptr = root->left_son_ptr;
    KD_TREE_NODE<PointType> * right_son_ptr = root->right_son_ptr;
...
template<typename PointType>
void KD_TREE<PointType>::flatten(KD_TREE_NODE<PointType> * root, PointVector &Storage, delete_point_storage_set storage_type){
    if (root == nullptr) return;
    Push_Down(root);
...
template<typename PointType>
void KD_TREE<PointType>::delete_tree_nodes(KD_TREE_NODE<PointType> ** root){ 
    if (*root == nullptr) return;
    Push_Down(*root);    
...
template<typename PointType>
float KD_TREE<PointType>::calc_box_dist(KD_TREE_NODE<PointType> * node, PointType point){
...
// Manual heap
template<typename PointType>
MANUAL_HEAP<PointType>::MANUAL_HEAP(int max_capacity){
    cap = max_capacity;
...
template<typename PointType>
MANUAL_HEAP<PointType>::~MANUAL_HEAP(){
    delete[] heap;
}

// 弹出最大的元素，也就是堆顶
template<typename PointType>
void MANUAL_HEAP<PointType>::pop(){
...
// 返回堆顶   
template<typename PointType>     
PointType_CMP<PointType> MANUAL_HEAP<PointType>::top(){
    return heap[0];
}

template<typename PointType>  
void MANUAL_HEAP<PointType>::push(PointType_CMP<PointType> point){
    // 如果堆的size大于设定容量，则不压入
...
template<typename PointType>     
int MANUAL_HEAP<PointType>::size(){
    return heap_size;
}

template<typename PointType>   
void MANUAL_HEAP<PointType>::clear(){
    heap_size = 0;
    return;
}

template<typename PointType>
void MANUAL_HEAP<PointType>::MoveDown(int heap_index){
    // 节点的左子树索引
...
template<typename PointType>       
void MANUAL_HEAP<PointType>::FloatUp(int heap_index){
    // 找到父节点
...
template<typename PointType>
Operation_Logger_Type<PointType> MANUAL_Q<PointType>::front(){
    return q[head];
}

template<typename PointType>
Operation_Logger_Type<PointType> MANUAL_Q<PointType>::back(){
    return q[tail];
}

template<typename PointType>
void MANUAL_Q<PointType>::push(Operation_Logger_Type<PointType> op){
    q[tail] = op;
    counter ++;
...
template<typename PointType>
bool MANUAL_Q<PointType>::empty(){
    return is_empty;
}

template<typename PointType>
int MANUAL_Q<PointType>::size(){
    return counter;
}
